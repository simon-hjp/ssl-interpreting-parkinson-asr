...
class CrossFullModel(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        # -- model architecture setup
        # If multiple SSL features, sum their input dims
        if isinstance(self.config.ssl_features, list):
            self.query_dim = sum([self.config.ssl_features_conf[feat]['input_dim'] for feat in self.config.ssl_features])
        else:
            self.query_dim = self.config.ssl_features_conf[self.config.ssl_features]['input_dim']

        self.key_dim = informed_input_dim
        self.value_dim = self.query_dim  # Assuming we are using the same dimensionality for value
    ...
    def forward(self, batch):
        ...
        # -- self-supervised speech representations (handles multiple SSL features)
        ssl_data = []
        for ssl_feat in self.config.ssl_features:
            ssl_data.append(batch[ssl_feat])  # -- (batch, time, D)
        ssl_data = torch.cat(ssl_data, dim=-1)  # Concatenate along the feature dimension

        # -- cross-embed attention
        all_embed_informed_data = all_informed_data.repeat(1, ssl_data.shape[1], 1)  # -- (batch, time, F)
        embed_mha_output = self.embed_mha(ssl_data, all_embed_informed_data, ssl_data, mask=None)  # -- (batch, time, D)
        reduced_embed_mha_output = Reduce('b n d -> b d', 'mean')(embed_mha_output)  # -- (batch, F)

        # -- cross-time attention
        all_time_informed_data = all_informed_data.repeat(1, ssl_data.shape[-1], 1)  # -- (batch, D, F)
        time_mha_output = self.time_mha(ssl_data, all_time_informed_data, ssl_data, batch['mask_ssl'])  # -- (batch, time, D)
        reduced_time_mha_output = Reduce('b n d -> b d', 'mean')(time_mha_output)  # -- (batch, F)

        # Concatenate the outputs from both attention heads
        mha_output = torch.cat([reduced_embed_mha_output, reduced_time_mha_output], dim=-1).unsqueeze(1)  # -- (batch, D*2)
